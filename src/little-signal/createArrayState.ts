...
export default function createArrayState<T extends Array<T[number]>>(value: T, name?: string): Array<SignalType<T[number]>> & SignalType<T> {
  const signals: Array<SignalType<T[number]>> = value.map((item, index) => littleSignal(item, `${name}[${index}]`))
    ...
    function (newValue?: T): T {
      ...
      newValue.forEach((item, index) => {
        if (signals[index]) {
          ...
          const newSignal = littleSignal(item, `${name}[${index}]`)
          newSignal._ls_.subscribers = previousSignalSubscribers
          signals[index] = newSignal
        } else {
          signals[index] = littleSignal(item, `${name}[${index}]`)
        }
      })
